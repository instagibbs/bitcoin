...
- `getprivatebroadcastinfo` now lists private broadcast jobs (`jobs`), each
  ...
  the node's own mempool has since seen the transaction, a running job's
  `progress`, and the finished job's full report. `abortprivatebroadcast` takes a job `id` instead of a
