...
void SetupArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-tor=<ip:port|path>", "Tor SOCKS5 listener: a loopback address with port, or a unix socket path (required). Seeds are resolved and recipients reached only through it.", ArgsManager::ALLOW_ANY | ArgsManager::DISALLOW_NEGATION, OptionsCategory::OPTIONS);
...
MAIN_FUNCTION
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            usage += "\n"
                     ...
                     "\n"
                     "Usage:  bitcoin-privbcast -tor=<ip:port> [options] send < tx.hex\n"
                     "or:     bitcoin-privbcast -tor=<ip:port> [options] discover\n";
            usage += "\n" + args.GetHelpMessage();
        }
    ...
    if (!cmd || (cmd->command != "send" && cmd->command != "discover") || !cmd->args.empty()) {
        tfm::format(std::cerr, "Error: expected exactly one command, 'send' or 'discover'\n");
        return EXIT_FAILURE;
    }

    std::string tor_str{args.GetArg("-tor", "")};
    if (tor_str.empty()) {
        tfm::format(std::cerr, "Error: -tor=<ip:port> is required\n");
        return EXIT_FAILURE;
    }
