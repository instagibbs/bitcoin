...
bool CBasicKeyStore::GetPubKey(const CKeyID &address, CPubKey &vchPubKeyOut) const
{
    ...
    if (!GetKey(address, key)) {
        LOCK(cs_KeyStore);
        PubKeyMap::const_iterator it = m_hww_pubkeys.find(address);
        if (it != m_hww_pubkeys.end()) {
...
{
    LOCK(cs_KeyStore);
    m_hww_pubkeys[pubkey.GetID()] = pubkey;
    ImplicitlyLearnRelatedKeyScripts(pubkey);
    return true;
}
...
{
    LOCK(cs_KeyStore);
    return mapKeys.count(address) > 0 || m_hww_pubkeys.count(address) > 0;
}
...
{
    LOCK(cs_KeyStore);
    PubKeyMap::const_iterator it = mapWatchKeys.find(keyId);
    if (it != mapWatchKeys.end()) {
        return true;
    }
    return false;
