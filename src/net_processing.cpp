...
static std::vector<std::pair<uint256, CTransactionRef>> vOrphanAndRecentlyRemovedTxn GUARDED_BY(cs_main);
...
void AddToCompactExtraTransactions(const CTransactionRef& tx)
{
    ...
    if (max_extra_txn <= 0)
        return;
    if (!vOrphanAndRecentlyRemovedTxn.size())
        vOrphanAndRecentlyRemovedTxn.resize(max_extra_txn);
    vOrphanAndRecentlyRemovedTxn[vOrphanAndRecentlyRemovedTxnIt] = std::make_pair(tx->GetWitnessHash(), tx);
    vOrphanAndRecentlyRemovedTxnIt = (vOrphanAndRecentlyRemovedTxnIt + 1) % max_extra_txn;
}
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (pindex->nHeight <= chainActive.Height() + 2) {
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                ReadStatus status = partialBlock.InitData(cmpctblock, vOrphanAndRecentlyRemovedTxn);
                ...
                ReadStatus status = tempBlock.InitData(cmpctblock, vOrphanAndRecentlyRemovedTxn);
