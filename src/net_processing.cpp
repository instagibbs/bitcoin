                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (next_task == InvalidTxTask::TRY_CPFP) {
            const auto maybe_cpfp_child = m_orphanage.MaybeGetSingleChild(ptx, pfrom.GetId());
            if (maybe_cpfp_child) {
                ...
                if (m_recent_rejects_reconsiderable.contains(GetPackageHash(maybe_cpfp_package))) {
                    LogPrint(BCLog::TXPACKAGES, "not trying package of parent %s (wtxid=%s) + child %s (wtxid=%s), found in recent rejects",
                             txid.ToString(), wtxid.ToString(), maybe_cpfp_child->GetHash().ToString(), maybe_cpfp_child->GetWitnessHash().ToString());
                } else {
                ...
                LogPrint(BCLog::TXPACKAGES, "not trying package for %s (wtxid=%s), no eligible child found", txid.ToString(), wtxid.ToString());
            }
        }
        return;
    }
