    ...
    for (const auto& txiter : cluster) {
        if (!m_to_be_replaced.count(txiter->GetTx().GetHash())) {
            auto [mapiter, success] = m_entries_by_txid.emplace(txiter->GetTx().GetHash(),
                MiniMinerMempoolEntry(/*feei=*/txiter->GetModifiedFee(), /*feea=*/txiter->GetModFeesWithAncestors(),
                                      /*sizei=*/txiter->GetTxSize(), /*sizea=*/txiter->GetSizeWithAncestors(), /*tx_in=*/txiter->GetSharedTx()));
