...
Package AncestorPackage::FilteredTxns() const
{
    ...
    for (const auto entryref : m_txns) {
        if (entryref.get().m_state == PackageEntry::State::PACKAGE) result.push_back(entryref.get().tx);
    }
    return result;
}
...
void AncestorPackage::IsInMempool(const CTransactionRef& transaction)
{
...
bool AncestorPackage::LinearizeWithFees()
{
    ...
    for (const auto& [txid, entry] : m_txid_to_entry) {
        ...
        auto anc_fee_vsize = FilteredAncestorFeeAndVsize(entry.tx);
        if (!Assume(anc_fee_vsize.has_value())) continue;
        miniminer_info.push_back(node::MiniMinerMempoolEntry{*entry.fee, anc_fee_vsize->first, *entry.vsize, anc_fee_vsize->second, entry.tx});
