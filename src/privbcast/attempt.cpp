                ...
                Clock::time_point hard_deadline, const std::function<bool()>& interrupted)
{
        ...
        while (!session.Finished()) {
            ...
            if (!queue.empty()) {
                ...
                if (transport.SetMessageToSend(queue.front())) {
                    queue.pop_front();
                    session.OnMessageHandedToTransport(type, Clock::now()); // the moment of acceptance
                    if (session.Finished()) return false;
                }
            }
            ...
            if (n > 0) {
                ...
                if (span_done && !type.empty()) {
                    ...
                    if (next.empty() || std::string{next_type} != type) {
                        session.OnMessageWritten(type, Clock::now());
                    }
                }
    ...
    while (!session.Finished()) {
        ...
        if (n > 0) {
            ...
            break;
        } else if (mocktime::Active()) {
            // A test's scripted peer has said all it will: only the clock can end the attempt now.
            WaitUntil(std::min(session.NextDeadline(), hard_deadline), interrupted);
        } else {
            // Nothing to read yet: poll the socket again in 10 ms of real time, whatever the clock says.
