...
DiscoveryResult Discover(const Proxy& tor, const DiscoveryPlan& plan, Clock::time_point t0, FastRandomContext& rng, const std::function<bool()>& interrupted)
{
    const size_t n{plan.dns_seeds.size()};
    ...
    Socks5Params socks{ToSteady(t0 + Scaled(disc::QUERY_DEADLINE))};
        ...
        for (uint32_t q = 0; q < disc::QUERIES_PER_SEED; ++q) {
            workers.emplace_back([st, i, q, tor, socks, name = plan.dns_seeds[i], t0, window_end, interrupted] {
                bool run{false};
                {
                    ...
                    if (!st->closed && !interrupted() && Clock::now() <= std::min(t0 + Scaled(disc::QUERY_GRACE), window_end)) {
                        ++st->queries[i];
                        run = true;
                    } else if (!st->closed) {
                ...
                if (run) {
                    try {
                        ...
                        std::lock_guard lock{st->mutex};
                        if (!st->closed && Clock::now() < window_end) { // late answers are discarded
                            if (addr) {
    ...
    std::vector<uint32_t> queries, skipped;
    {
        ...
        while (st->running > 0 && Clock::now() < window_end && !interrupted()) {
            st->cv.wait_until(lock, std::min(window_end, Clock::now() + 100ms));
        }
        st->closed = true;
        answers = st->answers;
        queries = st->queries;
    ...
    result.duration = std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now() - t0);
    return result;
}
