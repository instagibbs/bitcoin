...
static RPCMethod sendrawtransaction()
{
    return RPCMethod{
        ...
        "job starts, with no reaction to what the network does. This conceals the\n"
        "transaction's origin. The transaction will only enter the local mempool when it\n"
        "is received back from the network. The queue is bounded: when it is full, this RPC\n"
        "fails and the transaction is not queued. Use getprivatebroadcastinfo to inspect the\n"
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            if (private_broadcast_enabled && !node::PrivateBroadcastManager::UsableProxy(GetProxy(NET_ONION))) {
                throw JSONRPCError(RPC_MISC_ERROR,
                                   "-privatebroadcast is enabled, but no Tor SOCKS5 proxy on this host (a loopback "
                                   "address or a unix socket) is configured. Maybe the location of the Tor proxy "
                                       ...
                                       const std::vector<CTransactionRef>& txns, const CFeeRate& max_raw_tx_fee_rate)
{
    ...
    if (!node::PrivateBroadcastManager::UsableProxy(GetProxy(NET_ONION))) {
        throw JSONRPCError(RPC_MISC_ERROR, "-privatebroadcast is enabled, but no Tor SOCKS5 proxy on this host (a loopback address or a unix socket) is configured.");
    }
    if (txns.size() > 2) {
...
static RPCMethod submitpackage()
{
    return RPCMethod{"submitpackage",
        ...
        "package_msg is then \"parent-reconsiderable\" and private_broadcast_job the job id (see getprivatebroadcastinfo).\n"
        ,
        {
