...
/** Helper for OP_CHECKSIGFROMSTACK (BIP-348).
 *
 * Returns false when script execution must immediately fail. On success, success_out indicates
 * whether the signature check itself succeeded (false only for an empty signature).
 */
        ...
        for (; pc < pend; ++opcode_pos) {
            ...
            switch (opcode)
            {
                ...
                case OP_TEMPLATEHASH:
                {
                    ...
                    // returned in the default case, which would be the one hit by OP_SUCCESS206 before
                ...
                {
                    // DISCOURAGE/activation for OP_CHECKSIGFROMSTACK is handled in OP_SUCCESS handling.
...
#define FLAG_NAME(flag) {std::string(#flag), SCRIPT_VERIFY_##flag}
    static const std::map<std::string, script_verify_flag_name> g_names_to_enum{
        ...
        FLAG_NAME(TEMPLATEHASH),
        FLAG_NAME(DISCOURAGE_TEMPLATEHASH),
    };
#undef FLAG_NAME
