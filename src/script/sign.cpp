...
bool MutableTransactionSignatureCreator::CreateSchnorrSig(const SigningProvider& provider, std::vector<unsigned char>& sig, const VersionedXOnlyPubKey& ver_pubkey, const uint256* leaf_hash, const uint256* merkle_root, SigVersion sigversion) const
{
    ...
    uint256 hash;
    if (!SignatureHashSchnorr(hash, execdata, *txTo, nIn, nHashType, sigversion, ver_pubkey.version, *m_txdata, MissingDataBehavior::FAIL)) return false;
    sig.resize(64);
...
static bool SignTaproot(const SigningProvider& provider, const BaseSignatureCreator& creator, const WitnessV1Taproot& output, SignatureData& sigdata, std::vector<valtype>& result)
{
    ...
    // Try key path spending.
    {
        ...
        if (sigdata.taproot_key_path_sig.size() == 0) {
            if (creator.CreateSchnorrSig(provider, sig, {KeyVersion::TAPROOT, spenddata.internal_key}, nullptr, &spenddata.merkle_root, SigVersion::TAPROOT)) {
