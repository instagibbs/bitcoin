            ...
            [&] { // Add()
                ...
                if (present_before) {
                    ...
                    if (std::ranges::all_of(transactions, is_pending)) {
                        ...
                        // Find the evicted transaction: the only modelled one no longer in the queue.
                        const auto info{pb.GetBroadcastInfo()};
                        const auto evicted{std::ranges::find_if(transactions, [&](const auto& entry) {
                            return std::ranges::none_of(info, [&](const auto& e) { return CTransactionRefComp{}(e.tx, entry.first); });
                        })};
