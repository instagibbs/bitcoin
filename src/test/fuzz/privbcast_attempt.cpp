...
private:
    ...
    void OnToolMessage(const CNetMessage& msg) const
    {
        ...
        if (type == NetMsgType::VERSION) {
            const int version{m_fdp.PickValueInArray<int>({70016, 70016, 70015, 70001, 70000, 60002, m_fdp.ConsumeIntegral<int>()})};
            ...
            if (m_fdp.ConsumeBool()) Reply(NetMsg::Make(NetMsgType::WTXIDRELAY));
            ...
            {
                // The tool announces exactly its transaction: one txid INV, and only ever one.
                ++m_stats.inv_seen;
                DataStream is{msg.m_recv};
                std::vector<CInv> tool_inv;
                is >> tool_inv;
                assert(tool_inv.size() == 1 && tool_inv[0].type == MSG_TX && tool_inv[0].hash == txid);
            }
