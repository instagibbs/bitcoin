...
static constexpr int MAX_TX_IN_OUT{1'000};
...
FUZZ_TARGET(spendtemplatehash)
{
    ...
    if (!annex.empty()) {
        tx.vin[in_index].scriptWitness.stack.push_back(annex);  // TODO: a good way to test the target is to comment out this line
    }
    ...
    CallOneOf(
        ...
        // Adding an output changes sha_outputs and invalidates the template hash.
        [&] {
            tx.vout.emplace_back();
            Assert(!VerifyTemplateCheck(tx, in_index, std::move(spent_outputs), spent_spk));
        },
        // Removing an output changes sha_outputs and invalidates the template hash.
        [&] {
            if (tx.vout.empty()) return;
            tx.vout.pop_back();
            Assert(!VerifyTemplateCheck(tx, in_index, std::move(spent_outputs), spent_spk));
        },
        // Adding an input changes sha_sequences and invalidates the template hash.
        [&] {
            tx.vin.emplace_back();
            spent_outputs.emplace_back();
            Assert(!VerifyTemplateCheck(tx, in_index, std::move(spent_outputs), spent_spk));
        },
        // Removing an input changes sha_sequences and invalidates the template hash. Only the last
        // input is removed, so the index being spent stays valid and in place.
        [&] {
            if (tx.vin.size() < 2 || in_index + 1 == tx.vin.size()) return;
            tx.vin.pop_back();
            spent_outputs.pop_back();
            Assert(!VerifyTemplateCheck(tx, in_index, std::move(spent_outputs), spent_spk));
        }
    );
}
