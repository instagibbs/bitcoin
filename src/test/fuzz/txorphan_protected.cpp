...
FUZZ_TARGET(txorphan_protected, .init = initialize_protected_orphanage)
{
    ...
    // For now we have equal limits for all peers
    // We need to be able to set preferential peers
    // to have separate limits
    const unsigned int global_limit = 4'000;
    const unsigned int NUM_PEERS = 5;
    const unsigned int preferred_limit = global_limit / NUM_PEERS;
            ...
            CallOneOf(
                ...
                [&] {
                    ...
                    if (peer_is_protected && !have_tx_and_peer &&
                        orphanage.BytesFromPeer(peer_id) + tx_weight > preferred_limit) {
                ...
                [&] {
                    ...
                    // excluding expiry.
                    const auto protected_bytes{orphanage.BytesFromPeer(protect_peer)};
                    orphanage.LimitOrphans(global_limit, orphanage_rng);
                    Assert(orphanage.TotalOrphanBytes() <= global_limit);
                    // This should never differ before and after since we aren't allowing
                    // expiries.
                    Assert(protected_bytes == orphanage.BytesFromPeer(protect_peer));
                });
