...
FUZZ_TARGET(txorphan_protected, .init = initialize_protected_orphanage)
{
    ...
    const unsigned int NUM_PEERS = fuzzed_data_provider.ConsumeIntegralInRange<unsigned int>(1, DEFAULT_MAX_PEER_CONNECTIONS);
    const unsigned int global_announcement_limit = fuzzed_data_provider.ConsumeIntegralInRange<unsigned int>(NUM_PEERS, DEFAULT_MAX_ORPHAN_ANNOUNCEMENTS);
    ...
    const unsigned int per_peer_weight_reservation = fuzzed_data_provider.ConsumeIntegralInRange<unsigned int>(1, DEFAULT_RESERVED_ORPHAN_WEIGHT_PER_PEER * 10);
    ...
    LIMITED_WHILE(outpoints.size() < 200'000 && fuzzed_data_provider.ConsumeBool(), 10 * global_tx_limit)
    {
        ...
        const CTransactionRef tx = [&] {
            ...
            const auto num_out = fuzzed_data_provider.ConsumeIntegralInRange<uint32_t>(1, 1000);
