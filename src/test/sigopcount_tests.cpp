...
BOOST_AUTO_TEST_CASE(GetTxSigOpCost)
{
    ...
    // Multisig script (legacy counting)
    {
        ...
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), coins, flags).second == 0);
        // creationTx contains two signature operations in its scriptPubKey, but legacy counting
        // is not accurate.
        assert(GetTransactionSigOpCost(CTransaction(creationTx), coins, flags).second == MAX_PUBKEYS_PER_MULTISIG * WITNESS_SCALE_FACTOR);
    ...
    // Multisig nested in P2SH
    {
        ...
        BuildTxs(spendingTx, coins, creationTx, scriptPubKey, scriptSig, CScriptWitness());
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), coins, flags).second == 2 * WITNESS_SCALE_FACTOR);
        assert(VerifyWithFlag(CTransaction(creationTx), spendingTx, flags) == SCRIPT_ERR_CHECKMULTISIGVERIFY);
    }
    ...
    // P2WPKH witness program
    {
        ...
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), coins, flags).second == 1);
        // No signature operations if we don't verify the witness.
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), coins, flags & ~SCRIPT_VERIFY_WITNESS).second == 0);
    ...
    // P2WSH witness program
    {
        ...
        BuildTxs(spendingTx, coins, creationTx, scriptPubKey, scriptSig, scriptWitness);
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), coins, flags).second == 2);
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), coins, flags & ~SCRIPT_VERIFY_WITNESS).second == 0);
        assert(VerifyWithFlag(CTransaction(creationTx), spendingTx, flags) == SCRIPT_ERR_CHECKMULTISIGVERIFY);
    }
