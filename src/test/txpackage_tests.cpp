...
BOOST_FIXTURE_TEST_CASE(package_sanitization_tests, TestChain100Setup)
{
    ...
    BOOST_CHECK(!IsPackageWellFormed(package_conflicts, state_conflicts, /*require_sorted=*/false));
...
BOOST_FIXTURE_TEST_CASE(ancestorpackage, TestChain100Setup)
{
    ...
    // Basic chain of 25 transactions
    {
        ...
        for (auto i{0}; i < 10; ++i) packageified.IsInMempool(package[i]);
    ...
    // 99 Parents and 1 Child
    {
        ...
        for (auto i{0}; i < 99; ++i) {
            ...
            if (i < 50) packageified.IsInMempool(package.at(i));
        }
    ...
    packageified.IsInMempool(tx_20);
    ...
    }
    {
        ...
        packageified5.IsInMempool(grandparent_low_feerate);
        BOOST_CHECK(packageified5.LinearizeWithFees());
...
BOOST_FIXTURE_TEST_CASE(package_missing_inputs, TestChain100Setup)
{
    ...
    package_confirmed_parent.push_back(tx_confirmed_parent);
    ...
    auto mtx_child = CreateValidMempoolTransaction(/*input_transaction=*/tx_confirmed_parent, /*input_vout=*/0,
                                                   ...
                                                   /*output_amount=*/CAmount(48 * COIN), /*submit=*/false);
    CTransactionRef tx_child = MakeTransactionRef(mtx_child);
    package_confirmed_parent.push_back(tx_child);
    ...
    package_parent_dangles.push_back(tx_parent_dangle);
    ...
    package_parent_dangles.push_back(tx_parent_normal);
    ...
    auto tx_child_dangles{MakeTransactionRef(CreateValidMempoolTransaction(/*input_transactions=*/package_parent_dangles,
                                                                           ...
                                                                           /*submit=*/false))};
    package_parent_dangles.push_back(tx_child_dangles);
    ...
    BOOST_CHECK_MESSAGE(CheckPackageMempoolAcceptResult(package_confirmed_parent, result_confirmed_parent, /*expect_valid=*/false, m_node.mempool.get(), str), str);
    ...
    BOOST_CHECK_MESSAGE(CheckPackageMempoolAcceptResult(package_parent_dangles, result_parent_dangles, /*expect_valid=*/false, m_node.mempool.get(), str), str);
