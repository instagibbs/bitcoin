...
BOOST_FIXTURE_TEST_CASE(package_cpfp_tests, TestChain100Setup)
{
    ...
    BOOST_CHECK(m_node.mempool->GetMinFee().GetFee(GetVirtualTransactionSize(*tx_parent_cheap) + GetVirtualTransactionSize(*tx_child_cheap)) > 800);
...
BOOST_FIXTURE_TEST_CASE(package_rbf_tests, TestChain100Setup)
{
    ...
    // Test that V3 is required for package RBF, and not required for regular RBF.
    {
        ...
        BOOST_CHECK(submit3.m_tx_results.find(tx_parent_3->GetWitnessHash()) == submit3.m_tx_results.end());
        BOOST_CHECK(submit3.m_tx_results.find(tx_child_3->GetWitnessHash()) == submit3.m_tx_results.end());
