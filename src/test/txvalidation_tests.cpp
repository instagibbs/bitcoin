...
BOOST_FIXTURE_TEST_CASE(version3_tests, RegTestingSetup)
{
    ...
    // Cannot spend from an unconfirmed v3 transaction unless this tx is also v3.
    {
        ...
        BOOST_CHECK(*ApplyV3Rules(tx_v2_from_v3, *ancestors_v2_from_v3, /*num_other_ancestors=*/0, empty_conflicts_set, GetVirtualTransactionSize(*tx_v2_from_v3)) == expected_error_str);
        BOOST_CHECK(*PackageV3SanityChecks({mempool_tx_v3, tx_v2_from_v3}) == expected_error_str);
        ...
        BOOST_CHECK(*ApplyV3Rules(tx_v2_from_v2_and_v3, *ancestors_v2_from_both, /*num_other_ancestors=*/0, empty_conflicts_set, GetVirtualTransactionSize(*tx_v2_from_v2_and_v3))
                    == expected_error_str_2);
        BOOST_CHECK(*PackageV3SanityChecks({mempool_tx_v2, mempool_tx_v3, tx_v2_from_v2_and_v3}) == expected_error_str_2);
    }
    ...
    // V3 cannot spend from an unconfirmed non-v3 transaction.
    {
        ...
        BOOST_CHECK(*ApplyV3Rules(tx_v3_from_v2, *ancestors_v3_from_v2, /*num_other_ancestors=*/0, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_from_v2)) == expected_error_str);
        BOOST_CHECK(*PackageV3SanityChecks({mempool_tx_v2, tx_v3_from_v2}) == expected_error_str);
        ...
        BOOST_CHECK(*ApplyV3Rules(tx_v3_from_v2_and_v3, *ancestors_v3_from_both, /*num_other_ancestors=*/0, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_from_v2_and_v3))
                    == expected_error_str_2);
        BOOST_CHECK(*PackageV3SanityChecks({mempool_tx_v2, mempool_tx_v3, tx_v3_from_v2_and_v3}) == expected_error_str_2);
    }
    ...
    // Empty: no violations detected
    BOOST_CHECK(CheckV3Inheritance({}) == std::nullopt);

    // Just 1 transaction: no violations detected.
    BOOST_CHECK(CheckV3Inheritance({mempool_tx_v2}) == std::nullopt);

    // Duplicate transactions: no violations detected because no spending relationships between
    // these transactions.
    BOOST_CHECK(CheckV3Inheritance({mempool_tx_v3, mempool_tx_v3}) == std::nullopt);

    // Unrelated transactions: no violations detected because no spending relationships between
    // these transactions.
    BOOST_CHECK(CheckV3Inheritance({mempool_tx_v2, mempool_tx_v3}) == std::nullopt);

    // Putting them all together is fine, as spending relationships are checked.
    auto tx_v3_child_1 = make_tx({COutPoint{mempool_tx_v3->GetHash(), 0}}, /*version=*/3);
    auto tx_v2_child_1 = make_tx({COutPoint{mempool_tx_v2->GetHash(), 0}}, /*version=*/2);
    BOOST_CHECK(CheckV3Inheritance({mempool_tx_v2, tx_v2_child_1, mempool_tx_v3, tx_v3_child_1}) == std::nullopt);
    ...
    // Configuration where the tx has multiple direct parents.
    {
        ...
        BOOST_CHECK(ApplyV3Rules(tx_v3_multi_parent, *ancestors, /*num_other_ancestors=*/0, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_multi_parent)).has_value());
    }
    ...
    }
    {
        ...
        BOOST_CHECK(ApplyV3Rules(tx_v3_multi_gen, *ancestors, /*num_other_ancestors=*/0, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_multi_gen)).has_value());
    }
    ...
    many_inputs.emplace_back(mempool_tx_v3->GetHash(), 0);
    {
        ...
        BOOST_CHECK(ApplyV3Rules(tx_v3_child_big, *ancestors, /*num_other_ancestors=*/0, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_child_big)).has_value());
    }
    ...
    script_multisig << OP_2 << OP_CHECKMULTISIG;
    {
        ...
        BOOST_CHECK(ApplyV3Rules(tx_many_sigops, *ancestors, /*num_other_ancestors=*/0, empty_conflicts_set, bip141_vsize) == std::nullopt);
        // ...but sigop limit is.
        BOOST_CHECK(ApplyV3Rules(tx_many_sigops, *ancestors, /*num_other_ancestors=*/0, empty_conflicts_set, std::max(total_sigops * DEFAULT_BYTES_PER_SIGOP, bip141_vsize)).has_value());
    }
    ...
    auto tx_mempool_v3_child = make_tx({COutPoint{mempool_tx_v3->GetHash(), 0}}, /*version=*/3);
    {
        ...
        BOOST_CHECK(ApplyV3Rules(tx_mempool_v3_child, *ancestors, /*num_other_ancestors=*/0, empty_conflicts_set, GetVirtualTransactionSize(*tx_mempool_v3_child)) == std::nullopt);
    ...
    // A v3 transaction cannot have more than 1 descendant.
    {
        ...
        BOOST_CHECK(ApplyV3Rules(tx_v3_child2, *ancestors, /*num_other_ancestors=*/0, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_child2)).has_value());
        // If replacing the child, make sure there is no double-counting.
        BOOST_CHECK(ApplyV3Rules(tx_v3_child2, *ancestors, /*num_other_ancestors=*/0, {tx_mempool_v3_child->GetHash()}, GetVirtualTransactionSize(*tx_v3_child2)) == std::nullopt);
    }

    {
        ...
        BOOST_CHECK(ApplyV3Rules(tx_v3_grandchild, *ancestors, /*num_other_ancestors=*/0, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_grandchild)).has_value());
    }
}
