...
struct DepGraphFormatter
{
    ...
    void Unser(Stream& s, DepGraph<SetType>& depgraph)
    {
        ...
        /** Determine the set of unused positions in the currently-reconstructed cluster. */
        auto unused_fn = [&]() {
            SetType unused = SetType::Fill(SetType::Size());
            for (auto pos : reordering) unused.Reset(pos);
            return unused;
        };
        ...
        // Insert dummies in topo_depgraph that will map to the holes.
        SetType holes = unused_fn() & SetType::Fill(total_size);
        for (auto pos : holes) {
            topo_depgraph.AddTransaction(FeeFrac{});
            reordering.push_back(pos);
        }
