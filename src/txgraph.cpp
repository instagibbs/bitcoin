...
public:
    ...
    /** Populate the top ClusterSet's m_group_data based on its m_deps_to_add. */
    ...
    std::pair<std::vector<Ref*>, FeeFrac> GetWorstMainChunk() noexcept final;
...
bool TxGraphImpl::IsOversized(bool main_only) noexcept
{
    ...
    if (level == m_clustersets.size() - 1) ApplyRemovals(level);
...
void TxGraphImpl::SanityCheck() const
{
    ...
    for (size_t level = 0; level < m_clustersets.size(); ++level) {
        ...
        // on top is gone).
        if (level < m_clustersets.size() - 1) assert(clusterset.m_oversized.has_value());
    }
...
std::pair<std::vector<TxGraph::Ref*>, FeeFrac> TxGraphImpl::GetWorstMainChunk() noexcept
{
    std::pair<std::vector<Ref*>, FeeFrac> ret;
    // Make sure all clusters in main are up to date, and acceptable.
    SplitAll(0);
    if (m_clustersets.size() == 1) ApplyDependencies();
    MakeAllAcceptable(0);
