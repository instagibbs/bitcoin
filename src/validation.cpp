...
PackageMempoolAcceptResult MemPoolAccept::AcceptPackage(const Package& package, ATMPArgs& args)
{
    ...
    ATMPArgs single_args = ATMPArgs::SingleInPackageAccept(args);
    // Failures from calling PreChecks() on each transaction prior to package validation. If we
    // abort validation as a result of finding a non-TX_SINGLE_FAILURE error, this map can be used
    // to return the information we do have.
    ...
    auto final_submission_result = quit_early || linearized_package.FilteredTxns().empty() ?
    ...
    for (const auto& tx : package) {
        ...
        if (final_submission_result.m_tx_results.count(wtxid) > 0) {
            ...
            final_submission_result.m_tx_results.emplace(wtxid, MempoolAcceptResult::Failure(unknown_state));
        }
    }
