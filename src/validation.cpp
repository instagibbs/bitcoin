...
PackageMempoolAcceptResult MemPoolAccept::AcceptPackage(const Package& package, ATMPArgs& args)
{
        ...
        if (m_pool.exists(GenTxid::Wtxid(wtxid))) {
            ...
            if (PreChecks(single_args, ws)) {
                ...
                case TxValidationResult::TX_INPUTS_NOT_STANDARD:
                {
                    ...
                    //
                    // - Apart from TX_SINGLE_FAILURE (i.e. low feerate but CPFP'd), there is no way
    ...
    if (!quit_early) {
        ...
        for (const auto& tx : linearized_package.FilteredTxns()) {
            ...
            if (!subpackage_fee_vsize) continue;
            // FilteredAncestorSet should always be consistent with FilteredAncestorFeeAndVsize.
            ...
            if (rely_on_fees && !CheckFeeRate(subpackage_fee_vsize->second, subpackage_fee_vsize->first, placeholder_state)) {
            ...
            for (const auto& subpackage_tx : subpackage.value()) {
                ...
                if (m_pool.exists(GenTxid::Wtxid(subpackage_wtxid))) {
                    linearized_package.IsInMempool(subpackage_tx);
                }
            }
