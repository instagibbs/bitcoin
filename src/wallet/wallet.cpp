...
bool CWalletTx::IsTrusted() const
{
    ...
    for (const CTxIn& txin : tx->vin)
    {
        ...
        {
            // If the wallet is hdwatchonly, check if it is a key we generated or privkey imported
            // or based on a script that consists of those elements.
            if (!pwallet->IsHDWatchOnly() || isMine != ISMINE_WATCH_SOLVABLE)
                return false;
            const auto& meta = pwallet->mapKeyMetadata;
            auto it = meta.find(CScriptID(parentOut.scriptPubKey));
            // If no metadata can be found, it means it should not be in wallet
            if (it == meta.end()) {
                return false;
            } else if (it->second.hdKeypath.empty()) {
                // If path is empty, see if we know script preimage and
                // own *all* parts of redeemscript as hdwatchonly or imported privkeys.
                CScript redeemScript;
                std::vector<std::vector<unsigned char> > vSolutions;
                txnouttype whichType;
                // Only p2sh supported for now
                // Retrieve script hash and redeemscript
                if (!Solver(parentOut.scriptPubKey, whichType, vSolutions) || whichType != TX_SCRIPTHASH) {
                    return false;
                }
                if (!pwallet->GetCScript(CScriptID(uint160(vSolutions[0])), redeemScript)) {
                    return false;
                }
                txnouttype typeRet;
                std::vector<CTxDestination> addressRet;
                int nRequiredRet = -1;
                if (!ExtractDestinations(redeemScript, typeRet, addressRet, nRequiredRet)) {
                    return false;
                }
                // Iterate through addresses, fail if *any* key isn't generated by HD watchonly
                // wallet or hasn't had its privkey imported
                for (auto& dest : addressRet) {
                    CScript keyScript = GetScriptForDestination(dest);
                    it = meta.find(CScriptID(keyScript));
                    CKeyID keyID;
                    CBitcoinAddress(dest).GetKeyID(keyID);
                    if (it == meta.end() || (it->second.hdKeypath.empty() && !pwallet->HaveKey(keyID))) {
                        return false;
                    }
                }
            }
