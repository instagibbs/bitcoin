...
CPubKey CWallet::GenerateNewKey(WalletBatch &batch, bool internal)
{
    ...
    if (IsHDEnabled()) {
        DeriveNewChildKey(batch, metadata, secret, pubkey, (CanSupportFeature(FEATURE_HD_SPLIT) ? internal : false));
    } else {
    ...
    if (!secret.IsValid()) {
        if (!AddPubKeyWithDB(batch, pubkey)) {
            throw std::runtime_error(std::string(__func__) + ": AddPubKey failed");
        }
    } else if (!AddKeyPubKeyWithDB(batch, secret, pubkey)) {
...
std::shared_ptr<CWallet> CWallet::CreateWalletFromFile(const std::string& name, const fs::path& path, uint64_t wallet_creation_flags)
{
    ...
    CExtPubKey xpubkey;
    if (!externalHd.empty()) {
        xpubkey = DecodeExtPubKey(externalHd);
        if (!xpubkey.pubkey.IsFullyValid()) {
