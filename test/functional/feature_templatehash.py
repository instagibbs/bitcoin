...
class TemplateHashTest(BitcoinTestFramework):
    ...
    def test_discourage_no_disconnect(self):
        ...
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(commit_tx["tx"].txid_int, commit_tx["sent_vout"]), b"", SEQUENCE_FINAL))
    ...
    def test_basic(self):
        ...
        tx.vin[0].prevout.hash = commit_tx["tx"].txid_int
    ...
    def test_mutations(self):
        ...
        assert_raises_rpc_error(-25, "TestBlockValidity failed: block-script-verify-flag-failed (Script evaluated without error but finished with a false/empty top stack element)", self.generateblock, node, output="raw(51)", transactions=[commit_tx["hex"], tx.serialize().hex()])
        tx.version = 2
        assert node.testmempoolaccept([tx.serialize().hex()])[0]["allowed"]
        ...
        peer.send_and_ping(msg_tx(tx))
        assert_raises_rpc_error(-25, "TestBlockValidity failed: block-script-verify-flag-failed (Stack size must be exactly one after execution)", self.generateblock, node, output="raw(51)", transactions=[commit_tx["hex"], tx.serialize().hex()])
        tx.wit.vtxinwit[0].scriptWitness.stack = tx.wit.vtxinwit[0].scriptWitness.stack[1:]
        assert node.testmempoolaccept([tx.serialize().hex()])[0]["allowed"]
