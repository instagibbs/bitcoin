...
class EphemeralAnchorTest(BitcoinTestFramework):
    ...
    def test_node_restart(self):
        ...
        # Nodes restart; doesn't allow allow ephemeral transaction back in due to individual submission
        self.restart_node(0)
        self.restart_node(1)
    ...
    def test_fee_having_parent(self):
        ...
        node.submitpackage(package_hex1)
        self.assert_mempool_contents(expected=package_txns1, unexpected=[])
        self.sync_all() # Check for propagation # FIXME: "no eligible child found" at some point node send
        # a child with same parent, doesn't want to entertain it. Hacked to just pick one child.
    ...
    def test_prioritise_parent(self):
        ...
        node.submitpackage(package_hex1)
        self.assert_mempool_contents(expected=package_txns1, unexpected=[])
        self.sync_all() # Check for propagation
...
0)
    ...
    def test_sponsor_swap(self):
        ...
        assert_equal("ephemeral-anchor-unspent, tx does not spend all parent ephemeral anchors", res["tx-results"][child_a["wtxid"]]["error"])
