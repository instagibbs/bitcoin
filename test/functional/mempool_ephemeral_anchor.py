...
class EphemeralAnchorTest(BitcoinTestFramework):
    ...
    def create_simple_package(self, parent_coin, parent_fee=0, child_fee=DEFAULT_FEE, heavy_child=False, version=3, spend_anchor=1, additional_outputs=None):
    ...
    def test_unspent_ephemeral(self):
        ...
        package_hex0, package_txns0 = self.create_simple_package(parent_coin=parent_coin, parent_fee=0, child_fee=DEFAULT_FEE, version=3, spend_anchor=0)
        assert_raises_rpc_error(-26, "missing-ephemeral-spends", node.submitpackage, package_hex0)
        assert_equal(node.getrawmempool(), [])

        # Submit whole package with anchor spent, but wrong version!
        package_hex2, package_txns2 = self.create_simple_package(parent_coin=parent_coin, parent_fee=0, child_fee=DEFAULT_FEE, version=2)
        assert_raises_rpc_error(-26, "wrong-ephemeral-nversion", node.submitpackage, package_hex2)
        assert_equal(node.getrawmempool(), [])

        # One more time, correct nversions
        package_hex3, package_txns3 = self.create_simple_package(parent_coin=parent_coin, parent_fee=0, child_fee=DEFAULT_FEE, version=3)
    ...
    def test_fee_having_parent(self):
        ...
        package_hex0, package_txns0 = self.create_simple_package(parent_coin=parent_coin, parent_fee=1, child_fee=DEFAULT_FEE, version=3)
        assert_raises_rpc_error(-26, "invalid-ephemeral-fee", node.submitpackage, package_hex0)
        assert_equal(node.getrawmempool(), [])

        # But works with no parent fee
        package_hex1, package_txns1 = self.create_simple_package(parent_coin=parent_coin, parent_fee=0, child_fee=DEFAULT_FEE, version=3)
