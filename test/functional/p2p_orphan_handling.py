...
class OrphanHandlingTest(BitcoinTestFramework):
    ...
    def test_maximal_package_protected(self):
        ...
        for ancestor in ancestor_package[1:]:
            peer_normal.send_and_ping(msg_tx(ancestor["tx"]))

        orphan_set = node.getorphantxs()
        for ancestor in ancestor_package[1:]:
            assert ancestor["txid"] in orphan_set

        # Wait for ultimate parent request
        parent_txid_int = int(ancestor_package[0]["txid"], 16)
        node.bumpmocktime(NONPREF_PEER_TX_DELAY + TXID_RELAY_DELAY)
        ancestor_txid_int_list = [int(ancestor["txid"], 16) for ancestor in ancestor_package]
        ...
        self.log.info("Provide the ultimate parent. The whole package should be re-evaluated after enough time.")
        peer_normal.send_and_ping(msg_tx(ancestor_package[0]["tx"]))

        self.wait_until(lambda: node.getmempoolentry(ancestor_package[-1]["txid"])["ancestorcount"] == 25)
