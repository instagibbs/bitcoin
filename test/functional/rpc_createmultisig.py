...
class RpcCreateMultiSigTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        node0.createwallet(wallet_name='wmulti0', disable_private_keys=True, descriptors=self.options.descriptors)
        ...
        if 'wmulti' not in node1.listwallets():
            try:
                ...
                if e.error['code'] == -18 and 'Wallet wmulti not found' in e.error['message']:
                    node1.createwallet(wallet_name='wmulti', disable_private_keys=True, descriptors=self.options.descriptors)
                else:
                    raise
