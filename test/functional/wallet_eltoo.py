...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Both generated by CLN, we can just fund the spent output thanks to APO signature
        update_psbt = ""
        settle_psbt = ""

        decoded_update_psbt = self.nodes[0].decodepsbt(update_psbt)
        assert(decoded_update_psbt["fee"] == 0)

        eltoo_funding_addr = decoded_update_psbt["inputs"][0]["witness_utxo"]["scriptPubKey"]["address"]
        ...
        # First rebind the update tx to whatever the funding output ends up being
        update_raw_psbt = PSBT.from_base64(update_psbt)
        update_raw_tx = tx_from_hex(update_raw_psbt.g.map[0x00].hex())
        update_raw_tx.vin[0].prevout.hash = uint256_from_str(bytes.fromhex(funding_txid)[::-1])
        update_raw_tx.vin[0].prevout.n = [x['n'] for x in raw_funding_tx["vout"] if x["scriptPubKey"]["address"] == eltoo_funding_addr][0]
        ...
        final_tx = self.nodes[0].finalizepsbt(settle_psbt)
